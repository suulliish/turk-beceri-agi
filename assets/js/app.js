...
function countryFlagSrc(c){const f=(COUNTRIES||[]).find(x=>x.name===c);return f?f.flag:null;}
function countryFlagTag(c){const src=countryFlagSrc(c);return src?`<span class="mini-flag"><img src="${src}" alt="" loading="lazy"></span>`:'';}
...
function renderMatches(){const box=$('matchResults');if(!box)return;const filters={teach:$('matchTeach').value,learn:$('matchLearn').value,country:$('matchCountry').value,language:$('matchLanguage').value,level:$('matchLevel').value,availability:$('matchAvailability').value};const mode=$('matchMode').value;let list=DEMO_USERS.map(u=>scoreMatch(getProfile(),u,filters)).filter(u=>!filters.country||u.country===filters.country).filter(u=>!filters.language||u.languages.includes(filters.language)).filter(u=>!filters.level||u.level===filters.level).filter(u=>!filters.availability||u.availability===filters.availability);
 ...
 box.innerHTML=list.map(u=>{const r=u.rating?`${u.rating.toFixed(1)} / 5 · ${u.reviews} ${featureT('reviews')}`:featureT('noReviews');return `<article class="match-card"><div class="match-top"><div><h3 class="match-name">${countryFlagTag(u.country)}${escapeHtml(u.name)}</h3><div class="match-meta">${escapeHtml(countryLabel(u.country))} · ${escapeHtml(levelLabel(u.level))} · ${escapeHtml(availabilityLabel(u.availability))}</div></div><div class="match-score">${u.score}%</div></div><div class="match-tags">${u.teach.slice(0,3).map(x=>`<span>${escapeHtml(skillLabel(x))}</span>`).join('')}</div><div class="match-meta"><span class="star">★</span> ${r}</div><div class="match-reason"><strong>${featureT('whyMatch')}:</strong> ${u.reasons.length?u.reasons.join(' · '):featureT('demo')}</div><button class="ghost-btn" style="margin-top:11px" data-review-user="${u.id}">${featureT('rating')}</button></article>`}).join('');box.querySelectorAll('[data-review-user]').forEach(b=>b.onclick=()=>openReview(b.dataset.reviewUser));}
